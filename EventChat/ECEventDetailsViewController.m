//
//  ECEventDetailsViewController.m
//  EventChat
//
...
@interface ECEventDetailsViewController ()
...
@implementation ECEventDetailsViewController
...
- (void)viewDidLoad
{
    ...
    self.textInputbar.leftButton.hidden = NO;
...
- (void)configureDataSource
{
    ...
    // Get comments
    [[ECAPI sharedManager] fetchCommentsByTopicId:self.eventId callback:^(NSArray *comments, NSError *error) {
        if (error) {
        }
        else {
    ...
    //        message.content = [LoremIpsum wordsWithNumber:words];
    //        [array addObject:message];
    //    }
    //
    //    NSArray *reversed = [[array reverseObjectEnumerator] allObjects];
...
- (void)didPressRightButton:(id)sender
{
    ...
    NSLog(@"%@", self.eventId);
    //    [[ECAPI sharedManager] postComment:self.eventId userId:self.signedInUser.userId displayName:message.displayName content:message.content parentId:@"0" callback:^(NSDictionary *jsonDictionary, NSError *error){
    //        if (error) {
...
- (MessageTableViewCell *)messageCellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.titleLabel.text = message.displayName;
